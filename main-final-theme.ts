...
function updateHeading(doc: Document, options: any): void {
    const paragraphs = doc.getElementsByTagName("w:p");
    ...
    for (let i = 0; i < paragraphs.length; i++) {
        ...
        if (
            ...
            !headingTexts.includes(normalizedHeading)
        ) {
            continue;
        }
        ...
        if (textRuns.length > 0) {
            ...
            const firstT = textRuns[0].getElementsByTagName("w:t")[0];
            const lastT =
                textRuns[textRuns.length - 1].getElementsByTagName("w:t")[0];
...
function searchUpdate(doc: Document, options: any): void {
    if (!options.contentTexts || options.contentTexts.length === 0) {
        throw new Error("contentTexts[] is required for searchReplace mode");
    }
    ...
    for (let i = 0; i < paragraphs.length; i++) {
        ...
        for (let s = 0; s < normalizedSearchTexts.length; s++) {
            ...
            // refresh text after modification
            normalizedParaText = normalizeText(getParagraphText(p));
        }
    }

    console.log("Content search styling updated!");
}
...
function searchUpdateMain(doc: Document, options: any): void {
    if (!options.contentTexts || options.contentTexts.length === 0) {
        throw new Error("contentTexts[] is required for searchReplace mode");
    }
    ...
    console.log("Main content search styling updated!");
}
...
function replaceUpdate(
    ...
    for (let i = 0; i < paragraphs.length; i++) {
        ...
        if (!complete && isNonContentParagraph(p)) {
            continue;
        }
        ...
        applyStyleToRuns(Array.from(runs), doc, options);
    }
...
function processDocument(
    ...
    openAndSaveDocx(templatePath, (doc, themeDoc, stylesDoc) => {
        ...
        if (mode === "CONTENT") {
            searchUpdate(doc, options);
            return;
        }

        if (mode === "COMPLETE") {
            replaceUpdate(doc, options, true);
            return;
        }

        if (mode === "COMPLETE_CONTENT") {
            replaceUpdate(doc, options, false);
            return;
        }

        if (mode === "MAIN") {
            searchUpdateMain(doc, options);
            return;
        }
...
function searchUpdateTable(doc: Document, options: Options) {
    const searches: SearchItem[] = options.contentTexts
        ? options.contentTexts.map((t) => ({
            raw: t,
            normalized: normalizeText(t),
    ...
    console.log("Table styling updated!");
}
